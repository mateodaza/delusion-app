...
describe('Vitailik', function () {
  ...
  describe('Errors', function () {
    ...
    it('Cannot add selection for finished game', async () => {
      ...
      await expect(agent.addSelection(3, 0)).to.be.revertedWith(
        'Game is finished'
      );
    });
    it('Cannot add invalid selection', async () => {
      ...
      await expect(agent.addSelection(8, 0)).to.be.revertedWith(
        'Selection needs to be 0-3'
      );
    });
  });
});
